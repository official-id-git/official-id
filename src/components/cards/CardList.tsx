...
export function CardList({ cards, onDelete }: CardListProps) {
  ...
  const handleDeleteConfirm = async () => {
    ...
    const success = await deleteCard(pendingDelete.id)
    if (success && onDelete) {
  ...
  return (
    <>
        ...
        {cards.map(card => (
          <div key={card.id} className="bg-white rounded-lg shadow overflow-hidden">
            ...
            <div className="p-4 flex flex-wrap gap-2">
              ...
              <button
                onClick={() => handleDeleteClick(card.id, card.full_name)}
      ...
      <ConfirmModal
        ...
        message={`Yakin ingin menghapus kartu "${pendingDelete?.name}"? Tindakan ini tidak dapat dibatalkan.`}
