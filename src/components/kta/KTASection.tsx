...
export default function KTASection({ organizationId, organizationName, isMember }: KTASectionProps) {
    ...
    const handleDownload = async (type: 'pdf' | 'image', applicationId: string) => {
        try {
            ...
            // Get standard filename from headers or default to something safe
            const disposition = res.headers.get('content-disposition')
            let filename = `KTA_${formData.fullName.replace(/[^a-zA-Z0-9]/g, '_')}.${type === 'image' ? 'png' : 'pdf'}`
                    ...
                    <div className="space-y-5">
                        ...
                        <div className="flex flex-col sm:flex-row gap-3">
                            {myKTA.gdrive_pdf_url && (
                                <button
                                    onClick={() => handleDownload('pdf', myKTA.id)}
                            ...
                            {myKTA.gdrive_image_url && (
                                <button
                                    onClick={() => handleDownload('image', myKTA.id)}
