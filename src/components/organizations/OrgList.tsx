...
export function OrgList({ organizations, onDelete, showActions = true }: OrgListProps) {
  ...
  const handleDeleteConfirm = async () => {
    ...
    const success = await deleteOrganization(pendingDelete.id)
    if (success && onDelete) {
      onDelete(pendingDelete.id)
      showToast('Circle berhasil dihapus', 'success')
    } else if (!success) {
      showToast('Gagal menghapus Circle', 'error')
    }
    setDeletingId(null)
  ...
  return (
    <>
      ...
      <ConfirmModal
        isOpen={confirmOpen}
        title="Hapus Circle"
        message={`Yakin ingin menghapus organisasi "${pendingDelete?.name}"? Semua anggota akan dihapus.`}
